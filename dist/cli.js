...
import { getFilesNameFromDir, getFilterTypeInquirerString, getProductivityData, inquirerInputQustion, updateDb, } from "./lib.js";
console.log(chalk.bgBlue.white.bold(" Welcome to Typing Tracker (Made by www.hasanweb.tech) \n\n"));
export async function app(data) {
...
export async function getFilesForAction(data, action) {
...
export async function questionsForFilters(data, filterAction, fileName) {
    const answer = await inquirer.prompt([
        {
            type: "list",
            name: "value",
...
export async function lastQustionsForFilter(data, filterType, fileName) {
    const qustionStr = getFilterTypeInquirerString(filterType, {
        ...
        projectPath: `Enter the project name (example: twanis/backend), enter ${chalk.bgRed.white.bold("q")} to exit: `,
    });
    if (!qustionStr) {
...
export async function lastQustionsForFilterAllFromTo(data, filterType) {
    const qustionStr = getFilterTypeInquirerString(filterType, {
        regex: `Enter date value From to (example: 2022,1,1 2022,1,20), enter ${chalk.bgRed.white.bold("q")} to exit:`,
        year: `Enter the year From to (example: 2022 2024), enter ${chalk.bgRed.white.bold("q")} to exit:`,
